...
async function consultarSaldoCantv(numero:number) {
   ...
   // console.info("telefonoSinArea,",telefonoSinArea);
    ...
    const page = await initPage(urlApi);
...
function _removeToDatosElementsRepeatedCANTV(datos:any){
    ...
    */ 
    if(datos != null){
      delete (datos as any).saldoActual;
      delete (datos as any).fechaVencimiento;
      delete (datos as any).telefono;
      if(datos.error.toString().length ===0){
        delete (datos as any).error;
      }
    }
  ...
  export default consultarSaldoCantv;
